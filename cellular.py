...
In not only fixed bandwidth shells, but also cellular and lossy shells.
"""
import math
import sys
...
UPLINK_TRACE = "/home/ubuntu/mahimahi/traces/Verizon-LTE-short.down"
PORTUS_PATH = "/home/ubuntu/portus" # note: portus must be on branch for special slow start options
RESULTS_DIR = "/home/ubuntu/thesis_scripts/slow_start_results_hss"
...
def get_logname(alg, ccp, ext):
    if not ccp:
        return "cell-{}{}".format(alg, ext)
    else:
        return "cell-ccp{}{}".format(alg, ext)

def get_mahimahi_plot_title(alg, is_ccp):
    if not is_ccp:
        return "cell-{}".format(alg)
    else:
        return "cell-ccp-{}".format(alg)

def get_ccp_logname(alg):
    return "cell-{}-ccp.log".format(alg)
...
def run_individual_exp(alg, filename, is_ccp, results_folder):
    ...
    print "about to sleep before starting client"
    time.sleep(3)
...
def run_experiment(alg, filename, results_folder):
    ...
    cwnd_log = "cell-cwnd-{}.log".format(alg)
    cwnd_graph = "cell-cwnd-{}.pdf".format(alg)
...
def main():
    ...
    for alg in algs:
        ...
        run_experiment(alg, "25MB.html", results_folder) # reno on cellular link
